...
select continent, name from world a where a.name = (select name from world b where b.continent = a.continent order by name asc limit 1)

-- 9 Find the continents where all countries have a population <= 25000000. Then find the names of the countries associated with these continents. Show name, continent and population.

select name, continent, population from world x 
where 25000000 >= ALL(select y.population from world y where x.continent = y.continent and y.population > 0 ) 

-- 10 Some countries have populations more than three times that of any of their neighbours (in the same continent). Give the countries and continents.

select name, continent from world x 
where x.population/3 >  ALL(select y.population from world y where x.continent = y.continent and y.population > 0 and y.name != x.name) 
