...
SELECT name_question,
       name_answer,IF (is_correct, 'Верно', 'Неверно') as 'Результат' FROM answer INNER JOIN testing ON answer.answer_id = testing.answer_id 
       INNER JOIN question ON question.question_id = testing.question_id WHERE attempt_id = 7
