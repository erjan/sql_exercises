...
-- Указать, какой ответ дал студент и правильный он или нет.SELECT     name_question,
           name_answer,
           IF (is_correct, 'Верно', 'Неверно') AS ���������
FROM       answer
INNER JOIN testing
...
select     name_student,
           name_subject,
           date_attempt,
           round(sum(is_correct)/3 * 100,2) AS ���������
FROM       student
INNER JOIN attempt
ON         student.student_id = attempt.student_id
INNER JOIN testing
...
SELECT     name_subject,
           ...
           count(testing.answer_id)                                          AS �����_�������,
           round(100*( sum(answer.is_correct) / count(testing.answer_id)),2) AS ����������
FROM       answer
RIGHT JOIN question
